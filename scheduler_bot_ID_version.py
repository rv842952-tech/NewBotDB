...
def main():
    ...
    _manual_bot_id = os.environ.get('BOT_ID', '').strip()
    if _manual_bot_id:
        BOT_ID = _manual_bot_id
        logger.info(f"🔑 Using manual BOT_ID from env: {BOT_ID}")
    else:
        BOT_ID = db.make_bot_id(BOT_TOKEN)
        logger.info(f"🔑 BOT_ID derived from token: {BOT_ID}")
